...
exports.addArticleCates = (req, res) => {
    ...
    db.query(sql, [req.body.name, req.body.alias], (err, results) => {
        ...
        // 4.1 判断数据的 length，results.length === 2查询到两行数据分别被占用一个分类名称或分类别名
        if (results.length === 2) return res.cc('分类名称与分类别名被占用，请更换后重试！')
            // 4.2 length 等于 1 的三种情况，
            // results.length === 1查询到一行数据，1、分类名称和分类别名都被占用2、分类名称被占用3、分类别名被占用
...
exports.deleteCateById = (req, res) => {
    // 定义标记删除的 SQL 语句,并没有执行 DELETE 语句把数据删除掉，而是执行了 UPDATE 语句，将这条数据对应的 is_delete=1字段标记为删除即可
